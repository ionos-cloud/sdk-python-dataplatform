...
class MaintenanceWindow(object):
    ...
    def time(self):
        ...
        Time at which the maintenance should start. Must conform to the 'HH:MM:SS' 24-hour format.  # noqa: E501
    ...
    def time(self, time):
        ...
        if (self.local_vars_configuration.client_side_validation and
                time is not None and not re.search(r'^(?:[01]\d|2[0-3]):(?:[0-5]\d):(?:[0-5]\d)$', time)):  # noqa: E501
            raise ValueError(r"Invalid value for `time`, must be a follow pattern or equal to `/^(?:[01]\d|2[0-3]):(?:[0-5]\d):(?:[0-5]\d)$/`")  # noqa: E501
